<x-app-layout>
    <x-slot name="header">
        <div class="rounded-3xl border border-[#c7f0c7] bg-white/85 px-6 py-5 shadow-sm">
            ...
            <div class="mt-3">
                <h2 class="text-2xl font-semibold text-slate-900">Actualizar área</h2>
                <p class="mt-1 text-sm text-slate-600">Modifica los datos principales del área seleccionada.</p>
            </div>
        </div>
    </x-slot>
            ...
            <div class="rounded-3xl border border-[#d7f0d7] bg-white/95 p-8 shadow-lg shadow-[#d7f0d7]/30">
                <form method="POST" action="{{ route('areas.update', $area) }}" class="space-y-6">
                    ...
                    <div class="space-y-2">
                        ...
                        <x-form.input id="nombre" name="nombre" type="text" :value="old('nombre', $area->nombre)" required />
                        <x-form.error :messages="$errors->get('nombre')" />
                    </div>

                    <div class="space-y-2">
                        <x-form.label for="responsable" :value="__('Responsable (opcional)')" />
                        <x-form.input id="responsable" name="responsable" type="text" :value="old('responsable', $area->responsable)" />
